...
def test_tempdir_custom_path():
    ...
    with A_GIS.File.Directory.make(path=custom_path) as temp_dir:
        assert temp_dir.path == custom_path, "Directory path should match the custom path"
...
def test_tempdir_scoped_delete():
    ...
    with A_GIS.File.Directory.make(scoped_delete=True) as temp_dir:
        path = temp_dir.path
    assert not os.path.exists(path), "Directory should be deleted after exiting the context block"
...
def test_tempdir_no_delete():
    ...
    with A_GIS.File.Directory.make(scoped_delete=False) as temp_dir:
        path = temp_dir.path
    assert os.path.exists(path), "Directory should not be deleted after exiting the context block"
    # Clean up
    os.rmdir(path)
...
def test_tempdir_destructor():
    ...
    path = temp_dir.path
    del temp_dir
    assert not os.path.exists(path), "Directory should be deleted when the object is destructed"
