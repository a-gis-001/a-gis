    ...
    *,
    ...
    def get_embedding(text):
        if "instructor" in model.lower():
            # Instructor-XL requires an instruction
    ...
    token_counts = [
        len(tokenizer.encode(block, truncation=False)) for block in blocks
    ]
    ...
    similarity = [
        float(cos_sim(code_embedding, c)) for c in compare_embeddings
    ]
