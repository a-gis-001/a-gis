def remove_by_mask(*,image,mask,bg_color=255):
    ...
    # Convert the mask to a Boolean mask where True represents regions to discard
    ...
    if len(image_array.shape) == 3 and image_array.shape[2] == 3:
        boolean_mask_rgb = numpy.repeat(boolean_mask[:, :, numpy.newaxis], 3, axis=2)

        # Apply the Boolean mask: keep original values where mask is True
        masked_image[boolean_mask_rgb] = 255
    else:
        # If the image is grayscale (single channel), use the Boolean mask directly
