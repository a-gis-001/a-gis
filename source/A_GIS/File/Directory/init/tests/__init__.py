...
def test_tempdir_creation():
    ...
    with A_GIS.File.Directory.init() as temp_dir:
...
def test_tempdir_custom_path():
    ...
    with A_GIS.File.Directory.init(path=custom_path) as temp_dir:
        assert (
...
def test_tempdir_no_delete():
    ...
    with A_GIS.File.Directory.init(scoped_delete=False) as temp_dir:
        path = temp_dir.path
...
def test_tempdir_destructor():
    ...
    temp_dir = A_GIS.File.Directory.init(scoped_delete=True)
    path = temp_dir.path
    del temp_dir
    assert not os.path.exists(
        path
